...
# Merge rasters
rast_all_merged <- c(rast_all, rast_all2)
...
# Get species
spp3 <- list.dirs("more_models_richness/Projection_results/", 
                  recursive = FALSE, full.names = FALSE)

# Get variables and mask to AF
var <- rast("C:/Users/wever/Desktop/GitHub/napibio/Variables/Present/Variables.tif")
var_af <- crop(var, af, mask = TRUE)
plot(var_af$bio_1)

# Looping in the species
sp_l3 <- pblapply(seq_along(spp3), function(i){
  #Erro com Tillandsia_xiphioides
  try({
    #Get specie
    sp <- spp3[i]
    #print(sp)
    sp_dir <- file.path("more_models_richness/Models/", sp)
    ...
    #Get mask
    m_sp <- vect(file.path(sp_dir, "m.gpkg"))
    
    # Crop variables
    var_sp <- crop(var_af, m_sp, mask = TRUE) %>% trim()
    
    # Predict to AF
    p <- kuenm2::predict_selected(fm, raster_variables = var_sp, 
                                  consensus = "median",
                                  progress_bar = FALSE)
    # Binarize
    r10 <- (p$General_consensus$median > fm$thresholds$consensus$median) * 1
...
# Remove errors
r_all3 <- sp_l3[sapply(sp_l3, function(x) class(x) == "SpatRaster")]
# Rasterize
rast_all3 <- rast(r_all3)
# Merge data
# Resample
rast_all3_res <- resample(rast_all3, rast_all_merged, method = "near")
plot(rast_all3_res$`Zygopetalum mosenianum`)
# Merge
rast_all_merged2 <- c(rast_all_merged, rast_all3_res)

# Save TIFF file
writeRaster(rast_all_merged2, 
            "../GitHub/Richness_patterns/Data/Binarized_models.tiff")

# Convert to PAM
d <- terra::as.data.frame(rast_all_merged2, xy = TRUE)

#Save pam
library(data.table)
fwrite(d, 
       "C:/Users/wever/Desktop/GitHub/Richness_patterns/Data/PAM_final.gzip",
       compress = "gzip", row.names = FALSE)
